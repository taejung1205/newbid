...
export default function Index() {
  ...
  const [phoneNumber, setPhoneNumber] = useState<string>("Phone number undefined");
  const [email, setEmail] = useState<string>("Email undefined"); 
  const submit = useSubmit();

  useEffect(() => {
    requestUser({successCallback: (res: any) => {
      console.log("User: ");
      console.log(res);
  ...
  return (
    <MyBidPageBox>
      ...
      <h4>{phoneNumber}</h4>
      <h4>{email}</h4>
      <h4 onClick={() => requestUnlink({successCallback: (res: any) => {
        console.log("로그아웃 성공");
